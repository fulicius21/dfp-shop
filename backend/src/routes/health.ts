...
router.get('/detailed', asyncHandler(async (req, res) => {
  try {
    ...
    const serviceConfig = {
      ...
      email: {
        configured: !!(process.env.EMAIL_PROVIDER && process.env.SENDGRID_API_KEY)
      },
      features: {
        gdprCompliance: process.env.GDPR_ENABLED === 'true',
        rateLimit: !!(process.env.API_RATE_LIMIT_WINDOW_MS && process.env.API_RATE_LIMIT_MAX_REQUESTS),
        analytics: process.env.ANALYTICS_ENABLED === 'true',
        healthChecks: process.env.HEALTH_CHECK_ENABLED !== 'false'
      }
    };
...
/**
 * Erkennt den aktuellen Cloud-Provider
 */
function detectCloudProvider(): string {
  ...
  if (process.env.NETLIFY_SITE_ID) return 'Netlify';
